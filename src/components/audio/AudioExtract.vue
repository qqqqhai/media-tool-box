...
const handleStartExtract = async () => {
  if (fileList.value.length === 0) {
    ElMessage.warning('请先选择要提取音频的视频文件')
    return
  }
  ...
  try {
    ...
    for (let i = 0; i < fileList.value.length; i++) {
      ...
      try {
        // 1. 定义输入输出文件名
        const inputFileName = `video_input_${i}_${file.name}`
        // 替换文件扩展名为目标格式
    ...
    if (failList.length === 0) {
      ElMessage.success(`全部提取完成！成功处理${successCount}个视频文件`)
    } else {
